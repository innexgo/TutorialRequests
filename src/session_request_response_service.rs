...
pub async fn query(
  ...
  let sql = "
     ...
     AND ($4::bigint   IS NULL OR srr.creator_user_id = $4)
     AND ($5::text     IS NULL OR srr.message = $5)
     AND ($6::text     IS NULL OR srr.message LIKE CONCAT('%',$6,'%'))
     AND ($7::bool     IS NULL OR srr.committment_id IS NOT NULL = $7)
     AND ($8::bigint   IS NULL OR srr.committment_id = $8 IS TRUE)
     AND ($9::bigint   IS NULL OR sr.attendee_user_id = $9)
     AND ($10::bigint  IS NULL OR sr.course_id = $10)
     ...
     AND ($15::bigint  IS NULL OR c.session_id = $15)
